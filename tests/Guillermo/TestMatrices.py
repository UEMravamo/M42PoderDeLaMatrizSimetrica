...
ruta_csv = "C:\\Users\\guillermo\\Desktop\\4UE\\BigData\\PracticaGrupal\\rendimiento_matrices2.csv" 

with open(ruta_csv, mode='w', newline='') as archivo_csv:
    writer = csv.writer(archivo_csv)
    writer.writerow(["Dimension", "Potencia", "Tiempo Promedio (s)", "CPU Promedio (%)", "RAM Promedio (%)"])
...
def ram_usada():
    ...
    memoria_usada = proceso.memory_info().rss  # Cogemos la ram utilizada por el programa
...
def medir_rendimiento(dimension, exponente):
    ...
    print(f"Tiempo promedio: {tiempo_medio:.6f}s | CPU : {cpu_medio:.2f}% | RAM : {ram_medio:.2f}%")
    print("*******************************")

    # Guardamos los resultados en el archivo CSV
    with open(ruta_csv, mode='a', newline='') as archivo_csv:
        writer = csv.writer(archivo_csv)
        writer.writerow([dimension, exponente, tiempo_medio, cpu_medio, ram_medio])
